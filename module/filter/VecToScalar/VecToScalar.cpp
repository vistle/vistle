...
bool VecToScalar::compute() {
    ...
    switch (m_caseParam->getValue()) {
        case X: spec += "x"; out = extract(data_in, 0); break;
        case Y: spec += "y"; out = extract(data_in, 1); break;
        case Z: spec += "z"; out = extract(data_in, 2); break;
        case AbsoluteValue: spec += "magnitude"; out = calculateAbsolute(data_in); break;
    }
...
vistle::Vec<vistle::Scalar>::ptr
VecToScalar::extract(vistle::Vec<vistle::Scalar,3>::const_ptr &data, const vistle::Index &coord) {
...
vistle::Vec<vistle::Scalar>::ptr
VecToScalar::calculateAbsolute(vistle::Vec<vistle::Scalar,3>::const_ptr &data) {
#if VISTLE_WITH_VTKM
    try {
        ...
        mag.SetActiveField(
            fld,
            (data->mapping() == vistle::DataBase::Vertex)
    ...
    auto *o  = out->x().data();
    for (vistle::Index i=0; i<n; ++i) o[i] = std::sqrt(x[i]*x[i]+y[i]*y[i]+z[i]*z[i]);
    return out;
}
