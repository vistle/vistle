...
{
    m_gridFile = addStringParameter("grid_file", "File containing the grid","/mnt/raid/home/hpcleker/Desktop/MPAS/test40962/x1.40962.grid.nc", Parameter::Filename);
    m_dataFile = addStringParameter("data_file", "File containing data","", Parameter::Filename);
    m_zGridFile = addStringParameter("zGrid_file_dir", "File containing the vertical coordinates (elevation of cell from mean sea level","/", Parameter::Filename);
    m_partFile = addStringParameter("part_file", "File containing the grid partitions", "/mnt/raid/home/hpcleker/Desktop/MPAS/test40962/x1.40962.graph.info.part.8", Parameter::Filename);
   ...
   dataFileList.clear();
    if (hasDataFile){
        ...
        for (bf::directory_iterator it2(dataDir); it2 !=bf::directory_iterator();++it2) { //all files in dir
            if((bf::extension(it2->path().filename()) == ".nc") && (strncmp(dataFilePath.filename().c_str() ,it2->path().filename().c_str(), 15 )==0))
            {
...
bool ReadMPAS::read(Reader::Token &token, int timestep, int block)
{
    ...
    MPI_Offset numCells = dimCells.getSize();  //number of cells in 2D
    ...
    if (hasDataFile) {
        NcmpiFile ncDataFile(comm(), /*dataFileName.c_str()*/ dataFileList.at(timestep), NcmpiFile::read);
    ...
    const MPI_Offset &numLevels = std::min(numMaxLevels, numLevelsUser) + 1;
    if (numLevels < 2)
        return false;
    ...
    if (numPartsFile == 1) {
       /* size_t numCorn = std::accumulate(eoc.begin(), eoc.end(), 0);
        size_t numCornPlusOne = numCorn + numCells;
        UnstructuredGrid::ptr p(new UnstructuredGrid(
            numCells * (numLevels - 1), ((numLevels - 1) * numCornPlusOne * 2 + numCorn * (numLevels - 1) * 5),
            numVert * numLevels));
        Index *cl = p->cl().data();
        Index *el = p->el().data();
        auto tl = p->tl().data();
        ...
        for (Index iz = 0; iz < numLevels - 1; ++iz) {
            izVert = iz * numVert;
            for (Index elem = 0; elem < numCells; ++elem) {
                el[currentElem] = idx2;
                addCell(elem, el, cl, vPerC, numVert, izVert, idx2, voc);
                tl[currentElem++] = UnstructuredGrid::CPOLYHEDRON;
            }
        }
        el[currentElem] = idx2;

        std::vector<float> xCoords(numVert);
        std::vector<float> yCoords(numVert);
        std::vector<float> zCoords(numVert);
        Scalar *ptrOnX = p->x().data();
        Scalar *ptrOnY = p->y().data();
        Scalar *ptrOnZ = p->z().data();

        xC.getVar_all(start, stop, xCoords.data()); // xC->get(&xCoords[0], numVert);
        ...
        for (Index iz = 0; iz < numLevels; ++iz) {
            izVert2 = iz * numVert;
            for (Index iv = 0; iv < numVert; ++iv) {
                ptrOnX[izVert2 + iv] = (dH * iz + 1) * xCoords[iv];
                ptrOnY[izVert2 + iv] = (dH * iz + 1) * yCoords[iv];
                ptrOnZ[izVert2 + iv] = (dH * iz + 1) * zCoords[iv];
            }
        }

        geo_out = p;
        geo_out->setNumBlocks(numPartsFile);
        geo_out->updateInternals();
        token.addObject("grid_out", geo_out);

        for (Index port = 0; port < NUMPARAMS; ++port) {
            if (emptyValue(m_variables[port])) {
                continue;
            }

            NcmpiVar varData;
            if (hasDataFile) {
                NcmpiFile ncDataFile(comm(), dataFileName.c_str(), NcmpiFile::read);
                varData = ncDataFile.getVar(m_variables[port]->getValue().c_str());
            } else {
                varData = ncFirstFile.getVar(m_variables[port]->getValue().c_str());
                if (block == 0)
                    sendInfo("Using ncFirstFile as data source");
            }
            ...
            for (auto elem: varData.getDims()) {
                if (strcmp(elem.getName().c_str(), "nCells") == 0) {
                    ...
                    numElem.push_back(std::min(elem.getSize(), numLevels - 1));
                } else {
                    numElem.push_back(1);
                }
                startElem.push_back(0);
            ...
            std::vector<double> dataGet(numCells);
            Vec<Scalar>::ptr dataObj(new Vec<Scalar>(numCells * (numLevels - 1)));
            Scalar *ptrOnScalarData = dataObj->x().data();
            varData.getVar_all(startElem, numElem, dataGet.data());
            Index izCells = 0;
            for (Index iz = 0; iz < (numLevels - 1); ++iz) {
                izCells = iz * numCells;
                for (Index iv = 0; iv < numCells; ++iv) {
                    ptrOnScalarData[izCells + iv] = dataGet[iv * (numLevels-1) + iz];
                }
            }
            dataObj->setGrid(geo_out);
            dataObj->setNumBlocks(numPartsFile);
            dataObj->setMapping(DataBase::Element);
            std::string pVar = m_variables[port]->getValue();
            dataObj->addAttribute("_species", pVar);
            dataObj->updateInternals();
            token.addObject(m_dataOut[port], dataObj);
        }*/
        ...
        std::vector<Index> numCellsB(numPartsUser, 0);
        std::vector<Index> numCornB(numPartsUser, 0);
        std::vector<size_t> blockIdx(numPartsFile);

        for (Index i = 0; i < numPartsUser; ++i) {
            std::fill(blockIdx.begin() + (i * FileBlocksPerUserBlocks), blockIdx.begin() + ((i + 1) * FileBlocksPerUserBlocks), i);
        }
        if (remParts != 0)
        {
            for (Index i = 0; i < remParts; ++i) {
                std::fill(blockIdx.begin() + ((numPartsUser * FileBlocksPerUserBlocks)+i), blockIdx.begin() + ((numPartsUser * FileBlocksPerUserBlocks)+(i+1)), i);
            }
        }
            ...
            if (partList[i] == block) {
                for (Index d = 0; d < eoc[i]; ++d) {
                    if (vocIdxUsedTmp[voc[i * vPerC + d] - 1] < 0) {
                        ...
                        reducedVOC[i * MAX_EDGES + d] = idx; // is set to idx + 1 before it is increased -> idx+1-1 =idx
                    }else {
                        ...
                        if (partList[neighborIdx] != block) {
                            ...
                            for (Index dn = 0; dn < eoc[neighborIdx]; ++dn) {
                                if (vocIdxUsedTmp[voc[neighborIdx * vPerC + dn] - 1] < 0) {
                                    ...
                                    reducedVOC[neighborIdx * MAX_EDGES + dn] = vocIdxUsedTmp[voc[neighborIdx * vPerC + dn] - 1] + 1;
                                }
                            }
        ...
        NcmpiVar varData;
        Index dataIdx = 0;
        Vec<Scalar>::ptr dataObj(new Vec<Scalar>((numCellsB[block] + numGhosts) * (numLevels - 1)));
        Scalar *ptrOnScalarData = dataObj->x().data();
        ...
        if (!emptyValue(m_variables[dataIdx])) {
            if (hasDataFile) {
                ...
                getData(ncFirstFile,&dataValues,numLevels);
            }
        } else {
            if (block == 0)
        ...
        if (hasZData) { 
            ...
            for (int iz = 0; iz < numLevels; ++iz) {
                ...
                for (Index i = 0; i < numCells; ++i) {
                    ...
                    if ((partList[i] == block) || (ghosts && (isGhost[i] > 0))) {
                        for (Index d = 0; d < eoc[i]; ++d) {
                            ...
                            if (iv >= 0) {
                                ...
                                radius = (1./3.)*(zGrid[(numLevels)*i_v1+iz]+zGrid[(numLevels)*i_v2+iz]+zGrid[(numLevels)*i_v3+iz])+MSL; //compute vertex z from average of neighbouring cells
        ...
        dataValues.clear();
        //gridList[block] = p;
        geo_out = p; //gridList[block];
        geo_out->setBlock(block);
        geo_out->setTimestep(timestep); //TODO: read grid only once (in time -1)
