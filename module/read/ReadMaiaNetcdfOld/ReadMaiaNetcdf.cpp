...
bool ReadMaiaNetcdf::examine(const vistle::Parameter *param)
{
    ...
    auto reader = createReader(0, comm());
...
bool ReadMaiaNetcdf::read(Token &token, int timestep, int block)
{
  ...
  auto reader = createReader(block, *token.comm());
  if(m_isParticleFile){
...
std::unique_ptr<maiapv::ReaderBase> ReadMaiaNetcdf::createReader(int block, int mpiComm) const
{
