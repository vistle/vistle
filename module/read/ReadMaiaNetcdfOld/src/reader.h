...
template <MInt nDim> void Reader<nDim>::determineVisualizationLevelIndex(MInt* visLevels /*=-1*/) {
  ...
  for (MInt i = 0; i < m_maiaproxy->noInternalCells(); i++) {
    ...
    //   auto &maiaproxy = (*m_maiaproxy)[solverId_2];
    //   if (!maiaproxy->solverFlag(gridId, solverId_2)) continue;

    //   MInt solverCellId = maiaproxy->tree().grid2solver(gridId);
    //   // Only show non-leaf cells on the maximum visualization level
    //   if (m_leafCellsOn && (maiaproxy->tree().noChildren(solverCellId) == maiaproxy->m_maxNoChilds)) {
