...
void SzTecplotFile::Zone::ReadHeader(SzTecplotFile::Impl *pimpl, int iNumVar)
{
    ...
    if (pimpl->mVersion >= 112)
        ...
        mDataPacking = static_cast<PackingType>(pimpl->fetchInt32());
    ...
    if (mNeighborConnections != 0) {
        std::cerr << "User defined face neighbor mode (" << mNeighborConnections << ") not supported! \n";
...
MeshPts *SzTecplotFile::Zone::ReadInnerPtsData(SzTecplotFile::Impl *pimpl, int iNumVar)
{
    ...
    if (mType == ORDERED) {
        ...
        for (int p = 0; p < mNumPts * iNumVar; ++p) {
            int i, j;
            if (mDataPacking == BLOCK)
                j = p / mNumPts, i = p % mNumPts;
            else if (mDataPacking == POINT)
                j = p % iNumVar, i = p / iNumVar;
            else
                throw std::runtime_error("Unsupported DataPacking");
            if (j < 3)
                points[i][j] = pimpl->fetchFloat();
            else
                pimpl->fetchFloat(); // dummy read, as only coordinates are supported
        }
...
MeshBase *SzTecplotFile::Zone::ReadData(SzTecplotFile::Impl *pimpl, int iNumVar, RindSpec const &iRind)
{
    ...
    {
        if (rind.mMaxI > mIMax)
            std::cerr << "Rind specification error: rindMaxI (" << rind.mMaxI << ") > realMaxI (" << mIMax
                      << "), ignored\n",
                rind.mMaxI = mIMax;
        if (rind.mMaxJ > mJMax)
            std::cerr << "Rind specification error: rindMaxJ (" << rind.mMaxJ << ") > realMaxJ (" << mJMax
                      << "), ignored\n",
                rind.mMaxJ = mJMax;
        if (rind.mMaxK > mKMax)
            std::cerr << "Rind specification error: rindMaxK (" << rind.mMaxK << ") > realMaxK (" << mKMax
                      << "), ignored\n",
                rind.mMaxK = mKMax;
    }
    if (!rind.mMaxI)
        rind.mMaxI = mIMax;
    else if (rind.mMaxI < 0)
        rind.mMaxI = mIMax + rind.mMaxI;
    if (!rind.mMaxJ)
        rind.mMaxJ = mJMax;
    else if (rind.mMaxJ < 0)
        rind.mMaxJ = mJMax + rind.mMaxJ;
    if (!rind.mMaxK)
        rind.mMaxK = mKMax;
    else if (rind.mMaxK < 0)
        rind.mMaxK = mKMax + rind.mMaxK;
    if (mType == ORDERED) {
...
#ifdef MARENCO
        PointState *points(
            new PointState[is1D && pimpl->mDataSet != LIFT_LINE && pimpl->mDataSet != FLIGHTLAB_LINE ? mNumPts * 2
                                                                                                     : mNumPts]);
#elif defined(AHD)
        PointState *points(
            new PointState[is1D && pimpl->mDataSet != LIFT_LINE && pimpl->mDataSet != CAMRAD_LINE ? mNumPts * 2
                                                                                                  : mNumPts]);
#else
        ...
        for (int v = 0, readV = 0; readV < readNumPts * iNumVar; ++readV) {
            int readP, readM;
            if (mDataPacking == BLOCK)
                readM = readV / readNumPts, readP = readV % readNumPts;
            else if (mDataPacking == POINT)
                readM = readV % iNumVar, readP = readV / iNumVar;
            else
                throw std::runtime_error("Unsupported DataPacking");
            ...
            if (i < rind.mMinI || i >= rind.mMaxI || j < rind.mMinJ || j >= rind.mMaxJ || k < rind.mMinK ||
                k >= rind.mMaxK) {
            ...
            int p = 0, m = 0;
            if (mDataPacking == BLOCK)
                m = v / mNumPts, p = v % mNumPts;
            else if (mDataPacking == POINT)
                m = v % iNumVar, p = v / iNumVar;
            else
                throw std::runtime_error("Unsupported DataPacking");
            assert(m == readM);
            // clear surface velocity, if not to be read
            if ((iNumVar < 11 || pimpl->mDataSet == FLOWER_SURFACE || pimpl->mDataSet == FLOWER_PLANFILE ||
                 iNumVar == 13 || iNumVar == 14) &&
                m == 0)
                points[p].mV = 0;
            ...
            case FLOWER_PLANFILE:
                ...
                case 8:
                    points[p].mP = value;
                    break; // cp, in fact, has to be rescaled later on
                case 18:
                    assert(pimpl->mDataSet == FLOWER_SURFACE);
                    points[p].mN[0] = value;
                    break;
                case 19:
                    points[p].mN[1] = value;
                ...
                case 20:
                    ...
                    break;
                default:
                    break; /*assert(0);*/ // just skip rest, nothing more interesting
                }
                break;
            case FLOWER_NOVS:
            case FLOWER_ACCO:
            ...
            case LIFT_LINE:
                ...
                case 1:
                    points[p][0] = value;
                    break; // segment center, not corner
                case 2:
                    points[p][1] = value;
                    break;
                case 3:
                ...
                case 4:
                    points[p].mR = value;
                    break; // dr_ACP, not density
                case 5:
                    points[p].mP = value;
                    break; // clen_ACP, not pressure
                case 6:
                    points[p].mU.Z() = value;
                    break; // BEWARE: dT/dr
                case 7:
                    ...
                    break; // BEWARE: dD/dr
                case 8:
                    points[p].mU.X() = points[p].mR * points[p].mU.Z() * value;
                    break; // thrust direction, not fluid velocity
                case 9:
                    points[p].mU.Y() = points[p].mR * points[p].mU.Z() * value;
                    break; // using just read dr_ACP and dT/dr
                case 10:
                    points[p].mU.Z() = points[p].mR * points[p].mU.Z() * value;
                    break;
                case 11:
                    points[p].mV.X() = points[p].mR * points[p].mV.Z() * value;
                    break; // torque direction, not grid velocity
                case 12:
                    points[p].mV.Y() = points[p].mR * points[p].mV.Z() * value;
                    break; // using just read dr_ACP and dD/dr
                case 13:
                    points[p].mV.Z() = points[p].mR * points[p].mV.Z() * value;
                    break;
                default:
                    break;
                }
            ...
            case CAMRAD_SURFACE:
                ...
                case 8:
                    points[p].mP = value;
                    break; // quad area, not pressure
                default:
                    break; // nothing more interesting: psi_blade, r_ACP, x_ACP
                }
#else
            ...
            case CAMRAD_LINE:
                ...
                case 3:
                    points[p].mP = (value)*M_PI / 180;
                    break; // alpha
                case 4:
                    points[p].mR = value;
                    break; // dT
                case 5: {
                    points[p].mU.X() = points[p].mU.Y() = points[p].mV.Z() = 0; // dD
                    ...
                    points[p].mV.X() = points[p].Y();
                    points[p].mV.Y() = -points[p].X();
            ...
            case FLIGHTLAB_SURFACE:
                ...
                case 5:
                    points[p].mN.X() = value;
                    break; // quad normal, not fluid velocity
                case 6:
                    points[p].mN.Y() = value;
                    break;
                case 7:
        ...
        if (is1D) {
            ...
            if (pimpl->mDataSet == FLIGHTLAB_LINE) {
                ...
                LineTopo *cells(new LineTopo[++mNumElements]);
                mesh = new LineMesh<LineTopo>(mNumElements, mNumPts, cells, points);
            } else if (pimpl->mDataSet == IAGCOUPLE_ACCOLOAD) {
                LineTopo *cells(new LineTopo[++mNumElements]);
                mesh = new LineMesh<LineTopo>(mNumElements, mNumPts, cells, points);
            } else {
        ...
        for (int p = 0; p < mNumPts * iNumVar; ++p) {
            ...
            if (pimpl->mDataSet == FLOWER_ACCO && j > 7)
                continue;
            switch (j) {
            case 0:
                points[i][0] = value;
...
SzTecplotFile::SzTecplotFile(std::string const &iFile): mNumVar(0), mVarNames(0), pimpl(new Impl)
{
    ...
    while (marker != pimpl->EOHMARKER) {
        if (marker == pimpl->ZONEMARKER) {
            ...
            int numCoord = 2;
            if (position == 4 && pimpl->mVersion >= 101)
                numCoord = 3; // Grid3D
            ...
            if (geomType == 5 && pimpl->mVersion < 101)
                numCoord = 3; // 3DLine < V10.0
            pimpl->skip32(1);
            pimpl->skip64(2);
            pimpl->skip32(3);
            pimpl->skip64(2);
            ...
            if (geomType == 0 || (geomType == 5 && pimpl->mVersion < 101)) { // polyline || 3DLine
                ...
                while (numLines--) {
                    ...
                    if (gtype == 1)
                        pimpl->skip32(numCoord * numPoints);
                    else if (gtype == 2)
                        pimpl->skip64(numCoord * numPoints);
                    else
                        assert(0 == "illegal gtype value");
                }
                ...
                assert(0 == "illegal geometry type value");
            marker = pimpl->fetchFloat();
        } else if (marker == pimpl->TEXTMARKER) {
            assert(pimpl->mVersion >= 75 &&
                   "unchecked data layout for tecplot version < 9.2, check before removing assertion");
            //int position=
            pimpl->fetchInt32();
            pimpl->skip32(1);
            if (pimpl->mVersion < 101)
...
MeshBase *SzTecplotFile::ReadZone(size_t idx, const std::string &iZoneRindList)
{
    ...
    if (zoneNamePos != std::string::npos) {
        ...
        if (comma != std::string::npos && comma < endRind) {
            std::cerr << "Missing closing brace (early comma) in rind specification for zone " << i->getName()
                      << std::endl;
...
bool SzTecplotFile::checkDataSetType()
{
    ...
    // check variables
    if (mNumVar == 21) {
        if ((mVarNames[0] == std::string("CoordinateX")) && (mVarNames[1] == std::string("CoordinateY")) &&
            (mVarNames[2] == std::string("CoordinateZ")) && (mVarNames[3] == std::string("rho")) &&
            (mVarNames[4] == std::string("u")) && (mVarNames[5] == std::string("v")) &&
            (mVarNames[6] == std::string("w")) && (mVarNames[7] == std::string("Ma")) &&
            (mVarNames[8] == std::string("cp")) && (mVarNames[9] == std::string("YPlus")) &&
            (mVarNames[10] == std::string("TotalPressLoss")) && (mVarNames[11] == std::string("ViscosityEddy")) &&
            (mVarNames[12] == std::string("cfinf")) && (mVarNames[13] == std::string("cfloc")) &&
            (mVarNames[14] == std::string("LamTransFlag")) &&
            (mVarNames[15] == std::string("TurbulentEnergyKinetic")) &&
            (mVarNames[16] == std::string("TurbulentDissipationRate")) && (mVarNames[17] == std::string("iblank")) &&
            (mVarNames[18] == std::string("nx")) && (mVarNames[19] == std::string("ny")) &&
        ...
        if ((mVarNames[0] == std::string("CoordinateX")) && (mVarNames[1] == std::string("CoordinateY")) &&
            ...
            (mVarNames[4] == std::string("u")) && (mVarNames[5] == std::string("v")) &&
            (mVarNames[6] == std::string("w")) && (mVarNames[7] == std::string("Ma")) &&
            (mVarNames[8] == std::string("cp")) && (mVarNames[9] == std::string("Entropy")) &&
            (mVarNames[10] == std::string("TotalPressLoss")) && (mVarNames[11] == std::string("ViscosityEddy")) &&
            (mVarNames[12] == std::string("LamTransFlag")) && (mVarNames[13] == std::string("TurbulentDistance")) &&
            (mVarNames[14] == std::string("TurbulentEnergyKinetic")) &&
            (mVarNames[15] == std::string("TurbulentDissipationRate")) && (mVarNames[16] == std::string("iblank")))
            pimpl->mDataSet = FLOWER_PLANFILE;
    } else if (mNumVar == 11) {
        if ((mVarNames[0] == std::string("x")) && (mVarNames[1] == std::string("y")) &&
            (mVarNames[2] == std::string("z")) && (mVarNames[3] == std::string("rho")) &&
            (mVarNames[4] == std::string("u")) && (mVarNames[5] == std::string("v")) &&
            (mVarNames[6] == std::string("w")) && (mVarNames[7] == std::string("p")) &&
            (mVarNames[8] == std::string("us")) && (mVarNames[9] == std::string("vs")) &&
        ...
        if (((mVarNames[0] == std::string("x")) || (mVarNames[0] == std::string("CoordinateX"))) &&
            ...
            (mVarNames[3] == std::string("rho")) && (mVarNames[4] == std::string("u")) &&
            (mVarNames[5] == std::string("v")) && (mVarNames[6] == std::string("w")) &&
        ...
        if ((mVarNames[0] == std::string("CoordinateX")) && (mVarNames[1] == std::string("CoordinateY")) &&
            ...
            (mVarNames[4] == std::string("u")) && (mVarNames[5] == std::string("v")) &&
            (mVarNames[6] == std::string("w")) && (mVarNames[7] == std::string("p")) &&
            (mVarNames[8] == std::string("iblank")) && (mVarNames[9] == std::string("VortX")) &&
            (mVarNames[10] == std::string("VortY")) && (mVarNames[11] == std::string("VortZ")) &&
            (mVarNames[12] == std::string("Lambda2")))
            pimpl->mDataSet = FLOWER_ACCO;
    } else if (mNumVar == 14) {
        if ((mVarNames[0] == std::string("psi")) && (mVarNames[1] == std::string("QCx")) &&
            (mVarNames[2] == std::string("QCy")) && (mVarNames[3] == std::string("QCz")) &&
            (mVarNames[4] == std::string("dr")) && (mVarNames[5] == std::string("chord")) &&
            (mVarNames[6] == std::string("Fn")) && (mVarNames[7] == std::string("Ft")) &&
            (mVarNames[8] == std::string("Nx")) && (mVarNames[9] == std::string("Ny")) &&
            (mVarNames[10] == std::string("Nz")) && (mVarNames[11] == std::string("Tx")) &&
            (mVarNames[12] == std::string("Ty")) && (mVarNames[13] == std::string("Tz")))
        ...
        if (false) {
        } else if ((mVarNames[0] == std::string("Time [s]")) && (mVarNames[1] == std::string("Azimuth")) &&
                   (mVarNames[2] == std::string("x [m]")) && (mVarNames[3] == std::string("y [m]")) &&
                   (mVarNames[4] == std::string("z [m]")) && (mVarNames[5] == std::string("nx [-]")) &&
                   (mVarNames[6] == std::string("ny [-]")) && (mVarNames[7] == std::string("nz [-]")) &&
                   (mVarNames[8] == std::string("Area [m^2]")) && (mVarNames[9] == std::string("Psi [deg]")) &&
                   (mVarNames[10] == std::string("r_ACP [m]")) && (mVarNames[11] == std::string("x_ACP [-]")))
        ...
        if ((mVarNames[0] == std::string("time [s]")) && (mVarNames[1] == std::string("psi [deg]")) &&
            (mVarNames[2] == std::string("QCx_loc [m]")) && (mVarNames[3] == std::string("QCy_loc [m]")) &&
            (mVarNames[4] == std::string("QCz_loc [m]")) && (mVarNames[5] == std::string("Fx_loc [N]")) &&
            (mVarNames[6] == std::string("Fy_loc [N]")) && (mVarNames[7] == std::string("Fz_loc [N]")) &&
            (mVarNames[8] == std::string("A [m^2]")) && (mVarNames[9] == std::string("rho [kg/m^3]")))
            pimpl->mDataSet = IAGCOUPLE_ACCOLOAD;
        else if ((mVarNames[0] == std::string("x")) && (mVarNames[1] == std::string("y")) &&
                 (mVarNames[2] == std::string("z")) && (mVarNames[3] == std::string("Alpha")) &&
                 (mVarNames[4] == std::string("Lift")) && (mVarNames[5] == std::string("Drag")) &&
                 (mVarNames[6] == std::string("RadialForce")) && (mVarNames[7] == std::string("Radius")) &&
                 (mVarNames[8] == std::string("Chord")) && (mVarNames[9] == std::string("PanelWidth")))
        ...
        if ((mVarNames[0] == std::string("Time [s]")) && (mVarNames[1] == std::string("Azimuth")) &&
            (mVarNames[2] == std::string("Psi [deg]")) && (mVarNames[3] == std::string("r_ACP [m]")) &&
            (mVarNames[4] == std::string("x_ACP [-]")) && (mVarNames[5] == std::string("x_ACP1_IN [m]")) &&
            (mVarNames[6] == std::string("y_ACP1_IN [m]")) && (mVarNames[7] == std::string("z_ACP1_IN [m]")) &&
            (mVarNames[8] == std::string("dr_ACP [m]")) && (mVarNames[9] == std::string("clen_ACP [m]")) &&
            (mVarNames[10] == std::string("dT/dr [N/m]")) && (mVarNames[11] == std::string("dD/dr [N/m]")) &&
            (mVarNames[12] == std::string("Vvec_x_IN [-]")) && (mVarNames[13] == std::string("Vvec_y_IN [-]")) &&
            (mVarNames[14] == std::string("Vvec_z_IN [-]")) && (mVarNames[15] == std::string("Hvec_x_IN [-]")) &&
            (mVarNames[16] == std::string("Hvec_y_IN [-]")) && (mVarNames[17] == std::string("Hvec_z_IN [-]")))
            pimpl->mDataSet = FLIGHTLAB_LINE;
        else if ((mVarNames[0] == std::string("Time [s]")) && (mVarNames[1] == std::string("Azimuth")) &&
                 (mVarNames[2] == std::string("Psi [deg]")) && (mVarNames[3] == std::string("r_ACP [m]")) &&
                 (mVarNames[4] == std::string("x_ACP [-]")) && (mVarNames[5] == std::string("x_ACP1_HC [m]")) &&
                 (mVarNames[6] == std::string("y_ACP1_HC [m]")) && (mVarNames[7] == std::string("z_ACP1_HC [m]")) &&
                 (mVarNames[8] == std::string("dr_ACP [m]")) && (mVarNames[9] == std::string("clen_ACP [m]")) &&
                 (mVarNames[10] == std::string("dT/dr [N/m]")) && (mVarNames[11] == std::string("dD/dr [N/m]")) &&
                 (mVarNames[12] == std::string("Vvec_x_HC [-]")) && (mVarNames[13] == std::string("Vvec_y_HC [-]")) &&
                 (mVarNames[14] == std::string("Vvec_z_HC [-]")) && (mVarNames[15] == std::string("Hvec_x_HC [-]")) &&
                 (mVarNames[16] == std::string("Hvec_y_HC [-]")) && (mVarNames[17] == std::string("Hvec_z_HC [-]")))
            pimpl->mDataSet = FLIGHTLAB_LINE;
    }
...
MeshBaseVec Read(std::string const &iFile, std::string const & /*iZoneRindList*/)
{
    ...
    NodeMap_t **nodeMap = 0;
    double **vDataBase = 0;
    bool ok = ReadTec(FALSE /*GetHeaderInfoOnly*/, const_cast<char *>(iFile.c_str()), &fileVersion, &title, &numZones,
                      &numVars, &varNames, &zoneNames, &numPtsI, &numPtsJ, &numPtsK, &zoneType, 0 /*UserRec*/,
                      0 /*DatasetAuxData*/, FALSE /*RawDataspaceAllocated*/, &nodeMap, &vDataBase);
    SHOW_VAR4(ok, title, numZones, numVars);
    if (StringListCount(varNames) > 0)
        SHOW_VAR1(StringListGetStringRef(varNames, 0));
    if (StringListCount(varNames) > 1)
        SHOW_VAR1(StringListGetStringRef(varNames, 1));
    if (StringListCount(zoneNames) > 0)
        SHOW_VAR1(StringListGetStringRef(zoneNames, 0));
    if (StringListCount(zoneNames) > 1)
        SHOW_VAR1(StringListGetStringRef(zoneNames, 1));
    if (numZones > 0)
        SHOW_VAR3(numPtsI[0], numPtsJ[0], numPtsK[0]);
    if (numZones > 1)
        SHOW_VAR3(numPtsI[1], numPtsJ[1], numPtsK[1]);
