...
int64_t const DEFAULT_VALUES_PER_READ = 50;
...
int main(int argc, char** argv)
{
    if (argc != 3 && argc != 4)
    {
        std::cerr << "Usage: " << argv[0] << " infilename outfilename [numValuesPerRead]" << std::endl;
        return 1;
    }
    char* end;
    int64_t const numValuesPerRead = argc == 4 ? strtol(argv[3], &end, 10) : DEFAULT_VALUES_PER_READ;
        ...
        for (int32_t inputZone = 1; inputZone <= numZones; ++inputZone)
        {
            ...
            for (int32_t var = 1; var <= numVars; ++var)
            {
                if (passiveVarList[var - 1] == 0 && shareVarFromZone[var - 1] == 0)
                {
                    ...
                    while (numValuesRead < numValues)
                    {
                        int64_t numValuesToRead = std::min(numValuesPerRead, numValues - numValuesRead);
                        switch ((FieldDataType_e)varTypes[var - 1])
                        {
                        case FieldDataType_Float:
                        {
                            std::vector<float> values(numValuesPerRead);
                            i = tecZoneVarGetFloatValues(inputFileHandle, inputZone, var, numValuesRead + 1, numValuesToRead, &values[0]);
                            i = tecZoneVarWriteFloatValues(outputFileHandle, outputZone, var, 0, numValuesToRead, &values[0]);
                        }
                        break;
                        case FieldDataType_Double:
                        {
                            std::vector<double> values(numValuesPerRead);
                            i = tecZoneVarGetDoubleValues(inputFileHandle, inputZone, var, numValuesRead + 1, numValuesToRead, &values[0]);
                            i = tecZoneVarWriteDoubleValues(outputFileHandle, outputZone, var, 0, numValuesToRead, &values[0]);
                        }
                        break;
                        case FieldDataType_Int32:
                        {
                            std::vector<int32_t> values(numValuesPerRead);
                            i = tecZoneVarGetInt32Values(inputFileHandle, inputZone, var, numValuesRead + 1, numValuesToRead, &values[0]);
                            i = tecZoneVarWriteInt32Values(outputFileHandle, outputZone, var, 0, numValuesToRead, &values[0]);
                        }
                        break;
                        case FieldDataType_Int16:
                        {
                            std::vector<int16_t> values(numValuesPerRead);
                            i = tecZoneVarGetInt16Values(inputFileHandle, inputZone, var, numValuesRead + 1, numValuesToRead, &values[0]);
                            i = tecZoneVarWriteInt16Values(outputFileHandle, outputZone, var, 0, numValuesToRead, &values[0]);
                        }
                        break;
                        case FieldDataType_Byte:
                        {
                            std::vector<uint8_t> values(numValuesPerRead);
                            i = tecZoneVarGetUInt8Values(inputFileHandle, inputZone, var, numValuesRead + 1, numValuesToRead, &values[0]);
                            i = tecZoneVarWriteUInt8Values(outputFileHandle, outputZone, var, 0, numValuesToRead, &values[0]);
                        }
                        break;
                        default:
                            i = -1;
