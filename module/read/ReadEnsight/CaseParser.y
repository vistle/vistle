...
coord_change_spec: | CH_CO_ONLY
{
	driver.caseFile_.setConnectivityFileIndex(0);
}
| CH_CO_ONLY INTEGER
{
	driver.caseFile_.setConnectivityFileIndex($<token>2.iVal);
}

model_spec: MODEL any_identifier coord_change_spec
          {
		...
		//fprintf(stderr,"  ENSIGHT MODEL <%s> found\n", ensight_geofile.c_str());
          ...
          | MODEL INTEGER any_identifier coord_change_spec
          {
	  ...
	  // INTEGER now requires a whitespace, see lexer
          ...
          | MODEL INTEGER INTEGER any_identifier coord_change_spec
          {
          ...
          | MODEL INTEGER ASTNOTFN coord_change_spec
          {
	      std::string ensight_geofile($<token>3.szValue);
	      driver.caseFile_.setGeoFileNm( ensight_geofile );
	      int ts( $<token>2.iVal );
          ...
          | MODEL ASTNOTFN coord_change_spec
          {	      
	      std::string ensight_geofile($<token>2.szValue);
	      driver.caseFile_.setGeoFileNm( ensight_geofile );
          ...
          | MEASURED any_identifier coord_change_spec
          {
        	std::string ensight_geofile($<token>2.szValue);
	        driver.caseFile_.setMGeoFileNm( ensight_geofile );
          }
          | MEASURED INTEGER any_identifier coord_change_spec
          {
	      std::string ensight_geofile($<token>3.szValue);
	      driver.caseFile_.setMGeoFileNm( ensight_geofile );
	      int ts( $<token>2.iVal );
          ...
          | MEASURED INTEGER INTEGER any_identifier coord_change_spec
          {
	      std::string ensight_geofile($<token>4.szValue);
	      driver.caseFile_.setMGeoFileNm( ensight_geofile );
	      int ts( $<token>2.iVal );
          ...
          | MEASURED INTEGER ASTNOTFN coord_change_spec
          {
	      std::string ensight_geofile($<token>3.szValue);
	      driver.caseFile_.setMGeoFileNm( ensight_geofile );
	      int ts( $<token>2.iVal );
          ...
          // we may find lines like: model bla_geo.**** in this case we set the timeset to 1 
          | MEASURED ASTNOTFN coord_change_spec
          {	      
	      std::string ensight_geofile($<token>2.szValue);
	      driver.caseFile_.setMGeoFileNm( ensight_geofile );
	      driver.caseFile_.setGeoTsIdx(1);
...
variable_spec: var_pre IDENTIFIER STRING
	{
		...
		| var_pre VAR_POST
          ...
          {
				long ts($<token>2.iVal);
	      	      fprintf(stderr," VAR_INT VAR_POST ts=%ld %s\n", ts, $<token>3.szValue);
          ...
          {
				long ts($<token>2.iVal);
				long fs($<token>3.iVal);
	      	      fprintf(stderr," VAR_INT VAR_POST ts=%ld fs=%ld %s\n", ts, fs, $<token>4.szValue);
