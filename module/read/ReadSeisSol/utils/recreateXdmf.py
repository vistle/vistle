"""
Source: https://github.com/SeisSol/SeisSol/tree/master/postprocessing/visualization/tools

Changes: Add xdmf3-version change.
...
def recreateXdmf(prefix, prefix_new, nvertex, ncells, nmem, dt, indices, lsData, tohdf5=True, prec=8, append2prefix="_resampled", version=3, ndt=4):
    full_prefix = prefix
    prefix = os.path.basename(prefix)
...
<Xdmf Version="%d.0">
 <Domain>
  <Grid Name="TimeSeries" GridType="Collection" CollectionType="Temporal">""") % version
...
def ReadFromXdmf(subprocStr, attName):
    out = subprocess.check_output([subprocStr], shell=True)
    e = xml.etree.ElementTree.fromstring(out)
    return e.attrib[attName].split()


def ReadNcellsFromXdmf(xdmfFile):
    dimstring = ReadFromXdmf("grep connect " + xdmfFile + " | head -n1", "Dimensions")
    return int(dimstring[0])


def ReadDtFromXdmf(xdmfFile):
    dimstring = ReadFromXdmf("grep Time " + xdmfFile + " | head -n3| tail -n1", "Value")
    return float(dimstring[0])


def ReadNvertexFromXdmf(xdmfFile):
    dimstring = ReadFromXdmf("grep geometry " + xdmfFile + " | head -n1", "Dimensions")
    return int(dimstring[0])


def ReadNdtNmemFromXdmf(xdmfFile):
    dimstring = ReadFromXdmf("grep DataItem " + xdmfFile + " | tail -n2 | head -n1", "Dimensions")
...
if __name__ == "__main__":
    ...
    recreateXdmf(prefix, prefix, nvertex, ncells, nmem, dt, range(ndt), args.Data, version=args.version[0], ndt=ndt)
