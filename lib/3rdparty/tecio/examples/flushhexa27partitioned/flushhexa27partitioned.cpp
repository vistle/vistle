 ...
 *
 *   1.  Open a datafile called "flushhexa27partitioned.szplt"
 *       and "flushhexa27partitioned_solution.szplt"
    ...
    void**   outputFileHandle)
{
    ...
    if (returnValue == 0)
    {
        ...
        char const* baseFileName = numPartitions > 0 ? "flushhexa27partitioned" : "flushhexa27";
        char const* varNames;
        dataFileType = FULL;
    ...
    void*           outputFileHandle)
{
    ...
    INTEGER4 shareVarFromZone[] = {1, 1, 1, 0};
    INTEGER4 shrConn   = timeStep == 0 ? 0 : 1;

    if (timeStep == 0)
    {
        shareVarFromZone[0] = 0;
        shareVarFromZone[1] = 0;
        shareVarFromZone[2] = 0;
    }
    ...
    void*           outputFileHandle)
{
    ...
    // Add aux data to zones; for MPI, only the main output rank may do this.
#if defined TECIOMPI
    if (commRank == 0)
#endif
    {
        std::ostringstream auxDataValue;
        auxDataValue << timeStep;
        #if defined TECIOMPI
            if (commRank == 0)
        #endif
...
int main(int argc, char** argv)
{
    ...
    char const* NUM_TIME_STEPS_FLAG = "--num-time-steps";
    char* endptr = 0;
    int numTimeSteps = 5;
    if (argc == 3 && (strcmp(argv[1], NUM_TIME_STEPS_FLAG) == 0 &&
                      (strtol(argv[2], &endptr, 10) >= 0 && *endptr == 0)))
    {
        numTimeSteps = static_cast<int>(strtol(argv[2], &endptr, 10));
    }
    else if (argc != 1)
    {
        returnValue = 1;
        fprintf(stderr, "%s: Expected %s <count>, where <count> is 1 or greater.\n",
                argv[0], NUM_TIME_STEPS_FLAG);
    }
    ...
    if (returnValue == 0)
        returnValue = initializeFile(
                ...
                mpiComm,
            #endif
            numPartitions, &outputFileHandle);
    ...
    for (int timeStep = 0; returnValue == 0 && timeStep < numTimeSteps; ++timeStep)
    {
        ...
        if (returnValue == 0)
            returnValue = createZones(timeStep, numPartitions, partitionOwners, solTime, &outputZone, outputFileHandle);
        ...
        if (returnValue == 0)
            returnValue = createData(
                ...
                commRank,
                #endif
                timeStep, numPartitions, partitionOwners, solTime, outputZone, outputFileHandle);

        /*
         * Flush to disk.
         */
        int32_t zoneToRetain = 1;
        if (returnValue == 0)
            returnValue = tecFileWriterFlush(outputFileHandle, 1, &zoneToRetain);
