    ...
    void** fileHandle)
{
    ...
    varNames = "x y z p";
        ...
        int32_t debug = 1;
        int32_t vIsDouble = 1;
        int32_t returnValue = TECINI142("Flush Example Data Set", varNames, fileName.str().c_str(), ".", &fileFormat, &fileType, &debug, &vIsDouble);
    #else
        int32_t returnValue = tecFileWriterOpen(fileName.str().c_str(), "Flush Example Data Set", varNames, FILEFORMAT_SZL, FILETYPE_FULL, FieldDataType_Double, NULL, fileHandle);
        if (returnValue == 0)
    ...
    int32_t*        zone)
{
    ...
    int32_t* shareVarPtr = (timeStep == 0 ? NULL : shareVarFromZone);
        ...
        int32_t fNMode    = 0;
        int32_t zero = 0;
        int32_t returnValue = TECZNE142(zoneTitle.str().c_str(), &zoneType, &nNodes, &nCells, &nFaces, &iCellMax, &jCellMax, &kCellMax, &solTime, &strandID, &unused, &isBlock,
        ...
        int32_t returnValue = tecZoneCreateFE(fileHandle, zoneTitle.str().c_str(), ZONETYPE_FEBRICK, nNodes, nCells, NULL, shareVarPtr, valueLocations, NULL, shrConn, 0, 0, zone);
        if (returnValue == 0)
    ...
    int32_t         zone)
{
    ...
    #endif
    #if defined OLD_API
        returnValue = TECZAUXSTR142("TimeStep", timeStepString.str().c_str());
    #else
        returnValue = tecZoneAddAuxData(fileHandle, zone, "TimeStep", timeStepString.str().c_str());
    #endif
    ...
    timeStepLabel << "Time Step = " << timeStep;
    #if defined TECIOMPI
        if (commRank == 0)
    #endif
    {
        ...
        returnValue = TECTXT142(&xPos, &yPos, &zPos, &posCoordMode, &attachToZone, &zone,
            ...
            &textColor, &scope, &clipping, timeStepLabel.str().c_str(), MFC);
    #else
        if (returnValue == 0)
            returnValue = tecText3DBegin(fileHandle, timeStepLabel.str().c_str(), 5.0, 0.0, 8.0, 18.0, 2);
        if (returnValue == 0)
