...
static VisIt_SpeciesData *GetObject(visit_handle h, const char *fname)
{
    char tmp[100];
...
int simv2_SpeciesData_addSpeciesName(visit_handle h, visit_handle val)
{
    int retval = VISIT_ERROR;
...
int simv2_SpeciesData_setSpecies(visit_handle h, visit_handle val)
{
    ...
    void *data = NULL;
    if (simv2_VariableData_getData(val, owner, dataType, nComps, nTuples, data) == VISIT_ERROR) {
        return VISIT_ERROR;
    }
...
int simv2_SpeciesData_setSpeciesMF(visit_handle h, visit_handle val)
{
    // How many arrays make up the variable.
    int nArr = 1;
    if (simv2_VariableData_getNumArrays(val, &nArr) == VISIT_ERROR) {
...
int simv2_SpeciesData_setMixedSpecies(visit_handle h, visit_handle val)
{
    // How many arrays make up the variable.
    int nArr = 1;
    if (simv2_VariableData_getNumArrays(val, &nArr) == VISIT_ERROR) {
...
int simv2_SpeciesData_getData(visit_handle h, std::vector<visit_handle> &speciesNames, visit_handle &species,
