...
void *CopyStridedData(unsigned char *base, int nTuples, int offset, int &stride)
{
        ...
        if (memory == VISIT_MEMORY_CONTIGUOUS) {
            switch (dataType) {
                simV2TemplateMacro(
                    ...
                    realData = malloc(sz); if (realData) {
...
: VisIt_ObjectBase(VISIT_VARIABLE_DATA), arrays(), callback(NULL), callbackData(NULL)
{}
...
void VisIt_VariableData::SetArray(int arrId, VisIt_ArrayData *arr)
{
    pair<ArrayDataMapIt, bool> ins = arrays.insert(ArrayDataMap::value_type(arrId, arr));
...
VisIt_ArrayData *VisIt_VariableData::GetArray(int comp)
{
...
static int simv2_VariableData_setArrayData_internal(visit_handle h, int arrIndex, int memory, int owner, int dataType,
...

{
    ...
    if ((memory != VISIT_MEMORY_CONTIGUOUS) && (memory != VISIT_MEMORY_STRIDED)) {
        VisItError("The memory type must be either "
                   "VISIT_MEMORY_CONTIGUOUS or VISIT_MEMORY_STRIDED.");
        return VISIT_ERROR;
    }

    if ((owner != VISIT_OWNER_SIM) && (owner != VISIT_OWNER_VISIT) && (owner != VISIT_OWNER_VISIT_EX) &&
...
/* Set functions */
int simv2_VariableData_setData(visit_handle h, int owner, int dataType, int nComps, int nTuples, void *data)
{
    int arrIndex = 0;
    ...
    return simv2_VariableData_setArrayData_internal(h, arrIndex, memory, owner, dataType, nComps, nTuples, offset,
                                                    stride, data);
}

int simv2_VariableData_setDataEx(visit_handle h, int owner, int dataType, int nComps, int nTuples, void *data,
    ...
    if (owner == VISIT_OWNER_VISIT_EX) {
        ...
        if (!obj) {
            ...
            oss << "Error: " << __FILE__ << ":" << __LINE__ << std::endl
                << "Failed to locate the object from the given handle." << std::endl;
...
int simv2_VariableData_setArrayData(visit_handle h, int arrIndex, int owner, int dataType, int nTuples, int offset,
...
int simv2_VariableData_setDeletionCallback(visit_handle h, void (*callback)(void *), void *callbackData)
{
    int retval = VISIT_ERROR;
 ...
 */
int simv2_VariableData_getData2(visit_handle h, int *owner, int *dataType, int *nComps, int *nTuples, void **data)
{
    int arrIndex = 0;
    int memory = VISIT_MEMORY_CONTIGUOUS;
    int stride = 0, offset = 0;
    int retval = simv2_VariableData_getArrayData(h, arrIndex, memory, *owner, *dataType, *nComps, *nTuples, offset,
                                                 stride, *data);
#if 0
...
int simv2_VariableData_getArrayData(visit_handle h, int arrIndex, int &memory, int &owner, int &dataType, int &nComps,
...
int simv2_VariableData_getData(visit_handle h, int &owner, int &dataType, int &nComps, int &nTuples, void *&data)
{
    return simv2_VariableData_getData2(h, &owner, &dataType, &nComps, &nTuples, &data);
}

int simv2_VariableData_getDeletionCallback(visit_handle h, void (*&callback)(void *), void *&callbackData)
{
    int retval = VISIT_ERROR;
...
int simv2_VariableData_getNumArrays(visit_handle h, int *nArrays)
{
    int retval = VISIT_ERROR;
    *nArrays = 0;
...
int simv2_VariableData_nullData(visit_handle h)
{
    int retval = VISIT_ERROR;
