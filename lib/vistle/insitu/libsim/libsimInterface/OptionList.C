...
void SimV2Variant_setValue(SimV2Variant &var, int type, void *value)
{
    var.type = type;
    switch (type) {
...
void *SimV2Variant_getValue(SimV2Variant &var)
{
    void *ptr = NULL;
    switch (var.type) {
    case VISIT_DATATYPE_CHAR:
...
VisIt_OptionList::VisIt_OptionList(): VisIt_ObjectBase(VISIT_OPTIONLIST), keys(), values()
{}

VisIt_OptionList::~VisIt_OptionList()
{}

int VisIt_OptionList::index(const std::string &k) const
{
...
void VisIt_OptionList::setValue(const std::string &key, int type, void *value)
{
    int idx = index(key);
    if (idx == -1) {
        SimV2Variant var;
...
static VisIt_OptionList *GetObject(visit_handle h, const char *fname)
{
    char tmp[100];
...
int simv2_OptionList_setValue(visit_handle h, const char *name, int type, void *value)
{
    if (name == NULL) {
...
int simv2_OptionList_getNumValues(visit_handle h, int *nvalues)
{
    if (nvalues == NULL) {
...
int simv2_OptionList_getIndex(visit_handle h, const char *name, int *index)
{
...
int simv2_OptionList_getType(visit_handle h, int index, int *type)
{
    if (type == NULL) {
        VisItError("An invalid return variable was provided");
        return VISIT_ERROR;
...
int simv2_OptionList_getName(visit_handle h, int index, char **name)
{
    if (name == NULL) {
        VisItError("An invalid return variable was provided");
        return VISIT_ERROR;
...
int simv2_OptionList_getValue(visit_handle h, int index, void **value)
{
    if (value == NULL) {
        VisItError("An invalid return variable was provided");
        return VISIT_ERROR;
