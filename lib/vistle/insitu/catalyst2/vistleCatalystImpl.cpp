...
template <typename T>
void parseParam(const conduit_cpp::Node &params, T* target, const std::string &path, T (conduit_cpp::Node::*member)() const) {
  try
  {
...
void parseConnectionParams(const conduit_node *cparams) {
    ...
    parseParams(params, 
                  ConfigEntry<int>{&connectionParams.paused, "vistle/paused", &conduit_cpp::Node::to_int},
...
enum catalyst_status catalyst_execute_vistle(const conduit_node *cparams)
{
    ...
    if (!adapter) {
        adapter.reset(new vistle::insitu::Adapter(connectionParams.paused, connectionParams.comm,
                                                          std::move(metaData), cbs, VISTLE_ROOT, VISTLE_BUILD_TYPE,
