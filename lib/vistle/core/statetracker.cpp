...
ParameterSet StateTracker::getConnectedParameters(const Parameter &param, bool transitiveClosure) const
{
    mutex_locker guard(m_stateMutex);

    std::function<ParameterSet(const Port *, ParameterSet)> findAllConnectedPorts;
    findAllConnectedPorts = [this, &findAllConnectedPorts, transitiveClosure](const Port *port,
