...
bool Module::processMessagesSynced(ShmEnvelope &env, bool haveMessage, bool *messageReceived, unsigned int minPrio,
                                   bool *anyMessage)
{
    if (haveMessage) {
        recordMessage(env.message());
    }

    int sync = 0;
    if (haveMessage && needsSync(env.message())) {
        sync = env.message().type();
        assert(sync != 0);
    }
    ...
    while (true) {
        if (haveMessage) {
            ...
            sync = needsSync(env.message()) ? env.message().type() : 0;
            if (sync != 0 && allsync != sync) {
            ...
            quit = handleMessage(env) ? 0 : 1;
            if (quit) {
                CERR << "quitting after " << env.message() << std::endl;
                break;
            }
        }
        ...
        haveMessage = getNextMessage(env, true, minPrio);
...
bool Module::dispatch(bool block, bool *messageReceived, unsigned int minPrio)
{
    ...
    try {
        ...
        if (syncMessageProcessing()) {
            again = processMessagesSynced(pl, true, nullptr, minPrio);
        } else {
            again = handleMessage(pl);
            if (!again) {
                CERR << "quitting after " << pl.message() << std::endl;
            }
        }
