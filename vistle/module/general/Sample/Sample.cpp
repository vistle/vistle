...
DEFINE_ENUM_WITH_STRING_CONVERSIONS(ValueOutside,
      (NaN)
      (Zero)
      (userDefined)
      )
DEFINE_ENUM_WITH_STRING_CONVERSIONS(MultiHits,
      (Any)
      (Average)
      )
...
Sample::Sample(const std::string &name, int moduleID, mpi::communicator comm)
   : Module("MapDrape", name, moduleID, comm) {
    ...
    m_valOutside = addIntParameter("value_outside", "value to be used if target is outside source domain", Linear, Parameter::Choice);
    m_userDef =  addFloatParameter("user_defined_value", "user defined value if target outside source domain", 1.0);
    m_hits = addIntParameter("mulit_hits", "handling if multiple interpolatied values found for one target point ", Linear, Parameter::Choice);
...
int Sample::SampleToGrid(const vistle::GeometryInterface *target, vistle::DataBase::const_ptr inData, vistle::DataBase::ptr &sampled ) {
     int found =0;
...
bool Sample::reduce(int timestep) {
    ...
    for (int r=0; r<nProcs; ++r) {
     ...
     if (dataList.empty() || (timestep == dataList.at(0)->getNumTimesteps() - 1) || (dataList.at(0)->getNumTimesteps() < 2)) {
         dataList.clear();
         blockIdx.clear();
         objListLocal.clear();
     }
