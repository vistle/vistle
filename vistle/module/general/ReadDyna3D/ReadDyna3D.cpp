...
int ReadDyna3D::readDyna3D()
{
    ...
    CadfemFormat format = CadfemFormat(p_format->getValue());
    ...
    case GERMAN:
        ...
        std::cerr << "ReadDyna3D::readDyna3D info: selecting format " << ciform
             << std::endl;
        break;
    ...
    default:
        ...
        sendError("%s", buf);
        std::cerr << "ReadDyna3D::readDyna3D " << buf << std::endl;
        break;
    };
    ...
    else
    {
        ...
        for (istate = 1; istate < MAXTIMESTEPS; ++istate)
        {
            ...
            if ((istate >= MinState) && (istate <= State))
            {
                ...
                //	  std::cerr << "ReadDyna3D::readDyna3D() istate : " << istate << " flag: " << flag <<  std::endl;
                if (flag == -1)
                    return (-1);
    ...
    for (Index i = 0; i < NumSolidElements; i++)
        delete solidTab[i];
    delete[] solidTab;
    for (Index i = 0; i < NumTShellElements; i++)
        delete tshellTab[i];
    delete[] tshellTab;
    for (Index i = 0; i < NumShellElements; i++)
        delete shellTab[i];
    delete[] shellTab;
    for (Index i = 0; i < NumBeamElements; i++)
        delete beamTab[i];
    delete[] beamTab;
...
int ReadDyna3D::rdtaucntrl_(char *ciform)
{
    ...
    if ((byteswapFlag == BYTESWAP_AUTO) && ((version < 1) || (version > 10000)))
    {
        ...
        std::cerr << "ReadDyna3D::rdtaucntrl_ info: autobyteswap: "
    ...
    {
        std::cerr << "ReadDyna3D::rdtaucntrl_ info: autobyteswap: asuming byteswap off"
             << std::endl;
        byteswapFlag = BYTESWAP_OFF;
    }
   ...
   */
    ...
    sendInfo("File '%s': %i nodal points, %i solid elements, %i thick shell elements, %i shell elements, %i beam elements",
             data_Path.c_str(), numcoord, NumSolidElements, NumTShellElements, NumShellElements, NumBeamElements);
...
int ReadDyna3D::rdstate_(int *istate)
{
    ...
    if (!CEndin)
    {
        ...
        //      std::cerr << "ReadDyna3D::rdstate_(..) CEndin: " << CEndin << std::endl;
    ...
    if (*n > 0)
    {
        ...
        for (i = 1; i <= i__1; ++i)
        {
            ...
            if (irdst == 0)
            {
                ...
                {
                    std::cerr << "ReadDyna3D::rdreci_ info: Unknown format '"
                         << ciform << "' selected"
                         << std::endl;
                }
            }
...
int ReadDyna3D::otaurusr_()
{
    ...
    if (!COpenin)
    {
        ...
        strcpy(CTauin, data_Path.c_str());
// infile=Covise::open(CTauin,flags);
        infile = open(CTauin, flags);
        if (infile < 0) {
...
// infile=Covise::open(ctaun, flags);
#ifndef _AIRBUS
            infile = open(ctaun, flags);
            if (infile >= 0)
            {
...
void ReadDyna3D::createGeometry()
{
    ...
    for (Index ID = 0; ID < NumIDs; ID++)
    {
        ...
        if (IDLISTE[ID] && numelem[ID] > 0) {
            ...
            Index *el = &grid_out->el()[0];
            Index *cl = &grid_out->cl()[0];
            unsigned char *tl = &grid_out->tl()[0];
            Scalar *x_c = &grid_out->x()[0];
            Scalar *y_c = &grid_out->y()[0];
            Scalar *z_c = &grid_out->z()[0];
    ...
    for (ID = 0; ID < NumIDs; ID++)
    {
        ...
        if (IDLISTE[ID] && numelem[ID] > 0)
        {
...
#endif
            {
                ...
                for (i = minSolid[ID]; i <= maxSolid[ID]; i++)
                {
                    if (SolidMatIds[i] == ID && solidTab[i]->visible)
                    {
                ...
                for (i = minShell[ID]; i <= maxShell[ID]; i++)
                {
                    if (ShellMatIds[i] == ID && shellTab[i]->visible)
                    {
                        ++CoviseElement;
                        switch (shellTab[i]->coType)
                        {
                ...
                for (i = minBeam[ID]; i <= maxBeam[ID]; i++)
                {
                    if (BeamMatIds[i] == ID && beamTab[i]->visible)
                    {
...
bool ReadDyna3D::prepareRead() {
    ...
    {
        std::cerr << "ReadDyna3D::readDyna3D info: byteswap off" << std::endl;
    }
    else if (byteswapFlag == BYTESWAP_ON)
    {
        std::cerr << "ReadDyna3D::readDyna3D info: byteswap on" << std::endl;
    }
    else if (byteswapFlag == BYTESWAP_AUTO)
    {
        std::cerr << "ReadDyna3D::readDyna3D info: byteswap auto" << std::endl;
    }
    else
    {
        std::cerr << "ReadDyna3D::readDyna3D info: byteswap unknown error" << std::endl;
    }

    // Get the selection
    auto selection = p_Selection->getValue();
    coRestraint sel;
    sel.add(selection);
    ...
    Index numParts = 0;
    for (Index i = 0; i < MAXID; i++) {
        // Notice: offset for ID (= material number - 1)
        auto s = sel(i + 1);
        if (s > 0) {
            IDLISTE[i] = s;
            ++numParts;
    ...
    m_onlyGeometry = p_only_geometry->getValue();
...
bool ReadDyna3D::examine(const Parameter *param) {
    ...
    if (param == p_format || param == p_data_path) {

        // Covise::get_choice_param("format", &format);
        auto format = p_format->getValue();
        ...
        default:
            sendError("ERROR: Incorrect file format selected");
            return false;
            break;
        };

        if (p_data_path->getValue().empty()) {
            return false;
        }
