...
void PartitionReader::fillGrid(std::array<vistle::Scalar *, 3> mesh) const
{
    for (size_t i = 0; i < mesh.size(); i++) {
        if (sizeof(vistle::Scalar) == sizeof(double)) {
            std::transform(myGrid[i].begin(), myGrid[i].end(), mesh[i], [](float f) { return static_cast<double>(f); });
        } else {
            memcpy(mesh[i], myGrid[i].data(), sizeof(float) * gridSize);
        }
    }
}
...
bool PartitionReader::fillVelocity(int timestep, std::array<vistle::Scalar *, 3> data)
{
    if (m_hasVelocity) {
        ...
        for (size_t b = 0; b < myBlocksToRead.size(); ++b) {
            array<vector<float>, 3> velocity{vector<float>(blockSize), vector<float>(blockSize),
                                             vector<float>(blockSize)};
            if (!ReadVelocity(timestep, myBlocksToRead[b], velocity[0].data(), velocity[1].data(),
                              velocity[2].data())) {
                return false;
            }
...
bool PartitionReader::setPartition(int partition, unsigned numGhostLayers)
{
    ...
    int upper = lower + m_mapFileHeader[3] / numPartitions;
...
bool PartitionReader::ReadGrid(int timestep, int block, float *x, float *y, float *z)
{
    ...
    if (!CheckOpenFile(curOpenGridFile, timestep, fileID))
        return false;
    ...
    long iRealHeaderSize = m_headerSize + (isParallelFormat ? vBlocksPerFile[fileID] * sizeof(int) : 0);
    ...
    if (isBinary) {
        ...
        if (m_precision == 4) {
            fseek(curOpenGridFile->file(), iRealHeaderSize + (long)nFloatsInDomain * sizeof(float) * block, SEEK_SET);
            size_t res = fread(x, sizeof(float), blockSize, curOpenGridFile->file());
            (void)res;
            res = fread(y, sizeof(float), blockSize, curOpenGridFile->file());
            (void)res;
            if (dim == 3) {
                size_t res = fread(z, sizeof(float), blockSize, curOpenGridFile->file());
                (void)res;
            } else {
            ...
            fseek(curOpenGridFile->file(), iRealHeaderSize + (long)nFloatsInDomain * sizeof(double) * block, SEEK_SET);
            size_t res = fread(tmppts, sizeof(double), blockSize * dim, curOpenGridFile->file());
            (void)res;
            if (m_swapEndian)
        ...
        for (unsigned ii = 0; ii < blockSize; ii++) {
            fseek(curOpenGridFile->file(),
                  (long)curOpenGridFile->iAsciiFileStart +
                      (long)block * curOpenGridFile->iAsciiFileLineLen * blockSize +
                      (long)ii * curOpenGridFile->iAsciiFileLineLen,
                  SEEK_SET);
            if (dim == 3) {
                int res = fscanf(curOpenGridFile->file(), " %f %f %f", &x[ii], &y[ii], &z[ii]);
                (void)res;
            } else {
                int res = fscanf(curOpenGridFile->file(), " %f %f", &x[ii], &y[ii]);
                (void)res;
                memset(z, 0, blockSize * sizeof(float));
            }
        }
...
bool PartitionReader::ReadVelocity(int timestep, int block, float *x, float *y, float *z)
{
    ...
    if (isBinary) {
        ...
        if (m_precision == 4) {
            ...
            size_t res = fread(x, sizeof(float), blockSize, curOpenVarFile->file());
            (void)res;
            res = fread(y, sizeof(float), blockSize, curOpenVarFile->file());
            (void)res;
            if (dim == 3) {
                res = fread(z, sizeof(float), blockSize, curOpenVarFile->file());
                (void)res;
            } else {
                memset(z, 0, blockSize * sizeof(float));
            }
            ...
            double *tmppts = new double[blockSize * dim];
            fseek(curOpenVarFile->file(), filepos, SEEK_SET);
            size_t res = fread(tmppts, sizeof(double), blockSize * dim, curOpenVarFile->file());
            (void)res;
        ...
        for (unsigned ii = 0; ii < blockSize; ii++) {
            fseek(curOpenVarFile->file(),
                  (long)curOpenVarFile->iAsciiFileStart + (long)block * curOpenVarFile->iAsciiFileLineLen * blockSize +
                      (long)ii * curOpenVarFile->iAsciiFileLineLen + (long)dp.varOffsetAscii,
                  SEEK_SET);
            if (dim == 3) {
                int res = fscanf(curOpenVarFile->file(), " %f %f %f", x + ii, y + ii, z + ii);
                (void)res;
            } else {
                int res = fscanf(curOpenVarFile->file(), " %f %f", &x[ii], &y[ii]);
                (void)res;
                z[ii] = 0.0f;
            }
        }
...
bool PartitionReader::ReadVar(const string &varname, int timestep, int block, float *data)
{
    ...
    if (isBinary) {
        ...
        if (!isParallelFormat)
            ...
            if (strcmp(varname.c_str() + 2, "velocity") == 0) {
                filepos = (long)iRealHeaderSize + //header
                          (long)dp.timestepHasGrid * vBlocksPerFile[fileID] * blockSize * dim * m_precision + //grid
        ...
        if (m_precision == 4) {
            fseek(curOpenVarFile->file(), filepos, SEEK_SET);
            size_t res = fread(data, sizeof(float), blockSize, curOpenVarFile->file());
            (void)res;
            if (m_swapEndian)
            ...
            fseek(curOpenVarFile->file(), filepos, SEEK_SET);
            size_t res = fread(tmp, sizeof(double), blockSize, curOpenVarFile->file());
            (void)res;
            if (m_swapEndian)
        ...
        for (unsigned ii = 0; ii < blockSize; ii++) {
            fseek(curOpenVarFile->file(),
                  (long)curOpenVarFile->iAsciiFileStart + (long)block * curOpenVarFile->iAsciiFileLineLen * blockSize +
                      (long)ii * curOpenVarFile->iAsciiFileLineLen + (long)dp.varOffsetAscii,
                  SEEK_SET);
            int res = fscanf(curOpenVarFile->file(), " %f", var_tmp);
            (void)res;
            var_tmp++;
        }
    }
...
bool PartitionReader::ReadBlockLocations()
{
    ...
    for (int ii = 0; ii < numOutputDirs; ++ii) {
        ...
        for (int jj = ii * vBlocksPerFile[ii]; jj < (ii + 1) * vBlocksPerFile[ii]; ++jj) {
            ...
            if (iBlockID < 0 || iBlockID >= totalNumBlocks) {
...
void PartitionReader::addGhostBlocks(std::vector<int> &blocksNotToRead, unsigned numGhostLayers)
{
    ...
    for (size_t iterations = 0; iterations < numGhostLayers; iterations++) {
        ...
        for (int myBlock = 0; myBlock < oldNumBlocks; ++myBlock) {
            for (auto i = m_mapFileData[myBlocksToRead[myBlock]].begin() + 1;
                 i < m_mapFileData[myBlocksToRead[myBlock]].end(); i++) {
                {
                                                             ...
                                                             const std::array<std::vector<float>, 3> &currGrid)
{
    ...
    auto corners = m_mapFileData[myBlocksToRead[currBlock]]; //corner info starts at index 1
...
{
    auto corners = m_mapFileData[myBlocksToRead[localBlock]];
    for (int i = 1; i <= numCorners; i++) {
        allCorners[corners[i]] =
            connectivityList[blockIndexToConnectivityIndex.find(cornerIndexToBlockIndex(i))->second[0] +
                             localBlock * numCorners * hexesPerBlock];
    }
}
    ...
    for (auto edge: allEdgesInCornerIndices) {
        ...
        if (it == allEdges.end()) {
            ...
            for (int blockIndex: blockIndices) {
                gridIndices.push_back(connectivityList[blockIndexToConnectivityIndex.find(blockIndex)->second[0] +
                                                       localBlock * numCorners * hexesPerBlock]);
            }
