...
IsoSurface::IsoSurface(const std::string &shmname, int rank, int size, int moduleID)
   ...
   options.push_back("Plane");
   options.push_back("X-Cylinder");
   options.push_back("Y-Cylinder");
   options.push_back("Z-Cylinder");
   options.push_back("Sphere");
   setParameterChoices(m_option, options);
#endif    
#ifndef CUTTINGSURFACE
      ...
      case 0: {
         ...
         /*case 1: {
            ...
            DeviceData DD(m_isoValue,
                          ...
                          IsoDataFunctor(dataobj->x()),
              #else
                          IsoDataFunctor(vertex, point, m_grid->x().data(), m_grid->y().data(), m_grid->z().data()),
              #endif
...
#ifdef CUTTINGSURFACE
   void setCutData(const Vector m_vertex, const Vector m_point, const Vector m_direction){
      vertex = m_vertex;
      point = m_point;
