...
#if 0
   Triangles *triangles = new Triangles(Object::Initialized);
   triangles->setMeta(grid_object->meta());

   Vec<Scalar> *outData = new Vec<Scalar>(Object::Initialized);
      ...
      switch (tl[elem]) {
         ...
         case UnstructuredGrid::HEXAHEDRON: {
            ...
            for (int idx = 0; idx < 8; idx ++) {
               ...
               v[idx].y = y[index[idx]];
               v[idx].z = z[index[idx]];
               field[idx] = (normal * Vector(v[idx].x, v[idx].y, v[idx].z) - distance);
            }
...
#pragma omp critical
   ...
   return std::make_pair(Object::ptr(triangles), Object::ptr(outData));
}
#endif
