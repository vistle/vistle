...
class ReadBackCuda
{
   public:
      ...
      bool readdepthquant(GLint x, GLint y, GLint w, GLint pitch, GLint h,
            GLenum format, int ps, GLubyte *bits, GLint buf, GLenum type=GL_UNSIGNED_BYTE);

      static bool s_error; //!< whether a CUDA error occured
   private:
      bool initPbo(size_t raw, size_t compressed);
      bool pbo2cuda(GLint x, GLint y, GLint w, GLint pitch, GLint h,
...
ReadBackCuda::~ReadBackCuda() {}
bool ReadBackCuda::init() { return false; }
bool ReadBackCuda::isInitialized() const { return false; }
