...
namespace opencover
{
	...
	struct CubemapDrawer : public osg::Drawable::DrawCallback
	{
		CubemapReprojector* m_CubemapReprojector;

		CubemapDrawer(CubemapReprojector* cubemapReprojector)
			: m_CubemapReprojector(cubemapReprojector)
		{
		}

		void drawImplementation(osg::RenderInfo &ri, const osg::Drawable *d) const
		{
			m_CubemapReprojector->Render(ri.getContextID());
		}
	};
	...
	MultiChannelDrawer::~MultiChannelDrawer() {
		...
		for (size_t i = 0; i < m_channelData.size(); ++i)
			if(m_channelData[i].scene)
				removeChild(m_channelData[i].scene);
		m_channelData.clear();

		m_CubemapReprojector.Destroy();
	}
	...
	void MultiChannelDrawer::createGeometry(ChannelData &cd)
	{
		...
		cd.CubemapDrawable = new osg::Drawable();
		cd.CubemapDrawCB = new CubemapDrawer(&m_CubemapReprojector);
		cd.CubemapDrawable->setDrawCallback(cd.CubemapDrawCB);
	...
	{
		if (m_mode == Mode::ReprojectCubemap)
		{
			m_CubemapReprojector.SwapFrame();
		}
		else
		{
	...
	void MultiChannelDrawer::updateMatrices(int idx, const osg::Matrix &model, const osg::Matrix &view, const osg::Matrix &proj)
	{
		if (m_mode == Mode::ReprojectCubemap)
		{
			m_CubemapReprojector.SetNewServerCamera(idx, model.ptr(), view.ptr(), proj.ptr());
		}
		else
		{
	...
	void MultiChannelDrawer::resizeView(int idx, int w, int h, GLenum depthFormat)
	{
		if (m_mode == Mode::ReprojectCubemap)
		{
			m_CubemapReprojector.ResizeView(idx, w, h, depthFormat);
		}
		else
		{
			ChannelData &cd = m_channelData[idx];
...
void MultiChannelDrawer::reproject(int idx, const osg::Matrix &model, const osg::Matrix &view, const osg::Matrix &proj)
{
	...
	{
		assert(idx == 0);
		m_CubemapReprojector.SetClientCamera(model.ptr(), view.ptr(), proj.ptr());
	}
	else
	{
		ChannelData &cd = m_channelData[idx];
...
unsigned char *MultiChannelDrawer::rgba(int idx) const {
	if (m_mode == Mode::ReprojectCubemap)
		return const_cast<CubemapReprojector&>(m_CubemapReprojector).GetColorBuffer(idx);
...
{
	if (m_mode == Mode::ReprojectCubemap)
		return const_cast<CubemapReprojector&>(m_CubemapReprojector).GetDepthBuffer(idx);
...
void MultiChannelDrawer::setMode(MultiChannelDrawer::Mode mode) {
   ...
   for (size_t i=0; i<m_channelData.size(); ++i) {
	   ...
	   cd.geode->removeDrawable(cd.CubemapDrawable);

       if (mode == AsIs)
	   {
           cd.geode->addDrawable(cd.fixedGeo);
       } 
	   else if (mode == Mode::ReprojectCubemap)
	   {
		   cd.geode->addDrawable(cd.CubemapDrawable);
	   }
	   else
	   {
...
CubemapReprojector* MultiChannelDrawer::GetCubemapReprojector()
{
	return &m_CubemapReprojector;
}
