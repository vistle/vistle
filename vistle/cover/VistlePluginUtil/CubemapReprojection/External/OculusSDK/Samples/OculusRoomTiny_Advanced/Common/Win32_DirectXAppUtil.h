...
struct DepthBuffer
{
    ...
    DepthBuffer(ID3D11Device * Device, int sizeW, int sizeH, int sampleCount = 1)
    {
        ...
        ID3D11Texture2D * Tex;
        auto hr = Device->CreateTexture2D(&dsDesc, NULL, &Tex);
		VALIDATE((hr == ERROR_SUCCESS), "CreateTexture2D failed");
        hr = Device->CreateDepthStencilView(Tex, NULL, &TexDsv);
		VALIDATE((hr == ERROR_SUCCESS), "CreateDepthStencilView failed");
        Tex->Release();
    }
    ~DepthBuffer()
    {
	...
	// MUADDIB_ADD
	DepthBuffer(ID3D11DepthStencilView* texDsv)
	{
		this->TexDsv = texDsv;
	}
};
...
struct DataBuffer
{
    ...
    DataBuffer(ID3D11Device * Device, D3D11_BIND_FLAG use, const void* buffer, size_t size) : Size(size)
    {
        ...
        auto hr = Device->CreateBuffer(&desc, buffer ? &sr : NULL, &D3DBuffer);
		VALIDATE((hr == ERROR_SUCCESS), "CreateBuffer failed");
    }
    ~DataBuffer()
    {
        Release(D3DBuffer);
...
struct Texture
{
    ...
    void Init(int sizeW, int sizeH, bool rendertarget, int mipLevels, int sampleCount)
    {
        ...
        auto hr = DIRECTX.Device->CreateTexture2D(&dsDesc, NULL, &Tex);
		VALIDATE((hr == ERROR_SUCCESS), "CreateTexture2D failed");
		hr = DIRECTX.Device->CreateShaderResourceView(Tex, NULL, &TexSv);
		VALIDATE((hr == ERROR_SUCCESS), "CreateTexture2D failed");
        TexRtv = nullptr;
		if (rendertarget)
		{
			hr = DIRECTX.Device->CreateRenderTargetView(Tex, NULL, &TexRtv);
			VALIDATE((hr == ERROR_SUCCESS), "CreateRenderTargetView failed");
		}
    }
