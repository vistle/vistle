...
void CompositorIceT::screenBounds(int *bounds, const CompositeData &cd, const ViewData &vd, const osg::Vec3 &center, float r) {
    bounds[0] = cd.width;
    bounds[1] = cd.height;
    bounds[2] = 0;
    bounds[3] = 0;
...
struct CompositeCallback: public osg::Camera::DrawCallback {
    ...
    virtual void operator()(const osg::Camera &cam) const override {
        ...
        int width = cd.width;
        if (!CompositorIceT::readpixels(cd.cudaColor, vp[0], vp[1], vp[2], width, vp[3], GL_RGBA, 4, &cd.rgba[4*(vp[1]*width+vp[0])], cd.buffer, GL_UNSIGNED_BYTE)) {
            CERR << "read RGBA failed" << std::endl;
        }
        if (!CompositorIceT::readpixels(cd.cudaDepth, vp[0], vp[1], vp[2], width, vp[3], GL_DEPTH_COMPONENT, 4, reinterpret_cast<unsigned char *>(&cd.depth[vp[1]*width+vp[0]]), cd.buffer, GL_FLOAT)) {
...
#else
        cd.camera->setImplicitBufferAttachmentMask(osg::Camera::IMPLICIT_COLOR_BUFFER_ATTACHMENT|osg::Camera::IMPLICIT_DEPTH_BUFFER_ATTACHMENT,
...
void CompositorIceT::checkResize(int view)
{
   ...
   if (dim[0] != cd.width || dim[1] != cd.height) {
       CERR << "resizing view " << view << " from " << cd.width << "x" << cd.height << " to " << dim[0] << "x" << dim[1] << std::endl;
