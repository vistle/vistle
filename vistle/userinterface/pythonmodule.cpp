...
#include <control/communicator.h>
#include <control/modulemanager.h>
#define PORTMANAGER (Communicator::the().moduleManager().portManager())
...
#define LOCKED()
static std::map<std::string, int> outstandingSpawn;
...
static void sendMessage(const vistle::message::Message &m) {
   ...
   Communicator::the().commandQueue().send(m);
}
#else
static void sendMessage(const vistle::message::Message &m) {
...
#ifdef VISTLE_CONTROL
   auto it = outstandingSpawn.find(uuid);
   if (it != outstandingSpawn.end()) {
      int id = it->second;
      outstandingSpawn.erase(it);
      return id;
   }
#else
