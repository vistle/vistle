...
: m_moduleInfo(moduleInfo), m_rank(rank)
{
    vistle::message::DefaultSender::init(moduleInfo.id(), rank);
    // names are swapped relative to communicator
    std::string mqName = vistle::message::MessageQueue::createName(
    ("recvFromSim" + moduleInfo.uniqueSuffix()).c_str(), moduleInfo.id(), rank);
    try {
