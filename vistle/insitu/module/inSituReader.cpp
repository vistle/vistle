...
bool vistle::insitu::InSituReader::sendMessage(const vistle::message::Message &message, const buffer *payload) const
{
  if (payload && m_isExecuting) {
    CERR << "InSituReader: can't send message with payload while executing because that would create a vistle-object"
         << endl;
    return false;
  }
...
{
  std::string msqName =
      vistle::message::MessageQueue::createName(("recvFromSim" + std::to_string(m_numInstances)).c_str(), id(), rank());
