...
vistle::Object::ptr get(const visit_handle &meshHandle, message::SyncShmIDs &creator)
{
    ...
    if (check == VISIT_OKAY) {
        visit_handle coordHandles
        [4]; // handles to variable data, 4th entry contains interleaved data depending on
        // coordMode
        int dims[3]{1, 1, 1}; // the x,y,z dimensions
        int ndims, coordMode;
        // no v2check because last visit_handle can be invalid
        if (!simv2_CurvilinearMesh_getCoords(meshHandle, &ndims, dims, &coordMode, &coordHandles[0],
                                             &coordHandles[1], &coordHandles[2],
                                             &coordHandles[3])) {
        ...
        std::array<vistle::Scalar *, 3> gridCoords{mesh->x().data(), mesh->y().data(),
                                                   mesh->z().data()};
                                            ...
                                            bool vtkFormat)
{
    ...
    for (size_t iteration = 0; iteration < meshInfo.domains.size; iteration++) {
        ...
        int dims[3]{1, 1, 1}; // the x,y,z dimensions
        int ndims, coordMode;
        detail::getMeshCoord(meshInfo.domains.as<int>()[iteration], meshInfo.name, ndims, dims,
                             coordMode, coordHandles);

        size_t numVertices = getNumVertices(dims);
        size_t numElements = getNumElements(dims);

        allocateFields(mesh, totalNumVerts, numVertices, iteration, meshInfo.domains.size,
                       gridCoords, totalNumElements, numElements, numCorners);
        fillStructuredGridConnectivityList(
        dims, mesh->cl().begin() + totalNumElements * numCorners, totalNumVerts,
...
void separateFill(visit_handle coordHandles[4], int numCoords,
                  std::array<vistle::Scalar *, 3> &meshCoords, int dim)
{
    for (int i = 0; i < dim; ++i) {
        auto meshArray = getVariableData(coordHandles[i]);
        if (meshArray.size != static_cast<size_t>(numCoords)) {
            throw EngineExeption(
            "get(): received points in separate mesh do not match the mesh's dimension ")
            << i;
        }
...
void interleavedFill(visit_handle coordHandle, int numCoords,
                     const std::array<vistle::Scalar *, 3> &meshCoords, int dim)
{
    auto meshArray = getVariableData(coordHandle);
    if (meshArray.size != static_cast<size_t>(numCoords * dim)) {
        throw EngineExeption(
        "get(): received points in interleaved mesh do not match the mesh's dimensions");
    }

    transformInterleavedArray(meshArray.data, meshCoords, numCoords,
                              dataTypeToVistle(meshArray.type), dim);
}
