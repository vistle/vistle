...
struct Array
{
  ...
  template <typename T> const Iter<T> getIter() const { return Iter<T>(*this); }

  template <typename T> bool check() const { return dataTypeToVistle(type) == getDataType<T>(); }
...
template <typename T> void transformArray(const Array &source, T *dest)
{
