...
static void *plot_dlsym(const std::string &sym)
{
    void *retval = NULL;
#include <enabled_plots.h>
    return retval;
...
static void *operator_dlsym(const std::string &sym)
{
    void *retval = NULL;
#include <enabled_operators.h>
    return retval;
...
static void *database_dlsym(const std::string &sym)
{
    void *retval = NULL;
#ifdef HAVE_XDB
    PLUGIN_VERB(FieldViewXDB)
...
void *fake_dlsym(const std::string &sym)
{
    void *ptr = NULL;
...
void StaticGetSupportedLibs(std::vector<std::pair<std::string, std::string>> &libs, const std::string &pluginType)
{
    if (pluginType == "plot") {
        // Add the plots.
