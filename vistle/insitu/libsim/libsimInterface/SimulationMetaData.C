...
static VisIt_SimulationMetaData *GetObject(visit_handle h, const char *fname)
{
...
int simv2_SimulationMetaData_setMode(visit_handle h, int mode)
{
    if (mode != VISIT_SIMMODE_UNKNOWN && mode != VISIT_SIMMODE_RUNNING && mode != VISIT_SIMMODE_STOPPED) {
...
int simv2_SimulationMetaData_setCycleTime(visit_handle h, int cycle, double time)
{
    int retval = VISIT_ERROR;
...
#define ADD_METADATA(FUNC, EXPECTED_TYPE, OBJNAME, VEC) \
    int FUNC(visit_handle h, visit_handle obj) \
    { \
        int retval = VISIT_ERROR; \
        VisIt_SimulationMetaData *md = GetObject(h, #FUNC); \
        if (md != NULL) { \
            if (simv2_ObjectType(obj) != EXPECTED_TYPE) { \
                VisItError("An attempt was made to add a handle of a type other than " OBJNAME \
                           " to SimulationMetaData"); \
                return VISIT_ERROR; \
            } \
...
int simv2_SimulationMetaData_getData(visit_handle h, int &simulationMode, int &cycle, double &time)
{
    int retval = VISIT_ERROR;
...
#define GET_METADATA_ITEM(FUNC1, FUNC2, VEC) \
    int FUNC1(visit_handle h, int &n) \
    { \
        int retval = VISIT_ERROR; \
    ...
    int FUNC2(visit_handle h, int i, visit_handle &val) \
    { \
        int retval = VISIT_ERROR; \
...
GET_METADATA_ITEM(simv2_SimulationMetaData_getNumGenericCommands, simv2_SimulationMetaData_getGenericCommand,
                  genericCommands)
GET_METADATA_ITEM(simv2_SimulationMetaData_getNumCustomCommands, simv2_SimulationMetaData_getCustomCommand,
                  customCommands)
GET_METADATA_ITEM(simv2_SimulationMetaData_getNumMessages, simv2_SimulationMetaData_getMessage, messages)

int simv2_SimulationMetaData_check(visit_handle h)
{
    char tmp[128];
    int retval = VISIT_ERROR;
            ...
            if (simv2_VariableMetaData_check(obj->variables[i]) == VISIT_ERROR) {
                ...
                if (simv2_VariableMetaData_getMeshName(obj->variables[i], &name) == VISIT_OKAY) {
                    if (std::find(meshNames.begin(), meshNames.end(), name) == meshNames.end()) {
                        char *varName;
                        simv2_VariableMetaData_getName(obj->variables[i], &varName);
        ...
        for (i = 0; i < obj->materials.size(); ++i) {
            if (simv2_MaterialMetaData_check(obj->materials[i]) == VISIT_ERROR) {
                ...
                if (simv2_MaterialMetaData_getMeshName(obj->materials[i], &name) == VISIT_OKAY) {
                    ...
                    if (std::find(meshNames.begin(), meshNames.end(), name) == meshNames.end()) {
                        char *varName;
                        simv2_MaterialMetaData_getName(obj->variables[i], &varName);
        ...
        for (i = 0; i < obj->species.size(); ++i) {
            if (simv2_SpeciesMetaData_check(obj->species[i]) == VISIT_ERROR) {
                ...
                if (simv2_SpeciesMetaData_getMeshName(obj->species[i], &name) == VISIT_OKAY) {
                    if (std::find(meshNames.begin(), meshNames.end(), name) == meshNames.end()) {
                        char *varName;
                        simv2_SpeciesMetaData_getName(obj->species[i], &varName);
