...
template<int E>
class simV2_TypeTraits {};

#define simV2_TypeTraits_Specialize(visitE, cppT) \
    template<> \
    class simV2_TypeTraits<visitE> { \
    public: \
        typedef cppT cppType; \
    };

simV2_TypeTraits_Specialize(VISIT_DATATYPE_FLOAT, float) simV2_TypeTraits_Specialize(VISIT_DATATYPE_DOUBLE, double)
    simV2_TypeTraits_Specialize(VISIT_DATATYPE_INT, int) simV2_TypeTraits_Specialize(VISIT_DATATYPE_CHAR, unsigned char)
...
#define simV2TemplateMacroCase(visitE, call) \
    case visitE: { \
        typedef simV2_TypeTraits<visitE> simV2_TT; \
        call; \
    }; break
...
#define simV2FloatTemplateMacro(call) \
    ...
    inline bool simV2_ValidDataType(int dataType)
{
    if ((dataType == VISIT_DATATYPE_CHAR) || (dataType == VISIT_DATATYPE_INT) || (dataType == VISIT_DATATYPE_LONG) ||
        (dataType == VISIT_DATATYPE_FLOAT) || (dataType == VISIT_DATATYPE_DOUBLE)) {
        return true;
    }
    return false;
}
...
// verify that the type enum names one of the supported types
inline bool simV2_ValidFloatDataType(int dataType)
{
