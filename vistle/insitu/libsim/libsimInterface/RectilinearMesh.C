...
void VisIt_RectilinearMesh::FreeCoordinates()
{
...
void VisIt_RectilinearMesh::FreeGhostCells()
{
...
void VisIt_RectilinearMesh::FreeGhostNodes()
{
...
static VisIt_RectilinearMesh *GetObject(visit_handle h, const char *fname)
{
    char tmp[100];
...
static int simv2_RectilinearMesh_setCoords_helper(visit_handle h, visit_handle *cHandles, int ndims, const char *fname)
{
    int retval = VISIT_ERROR;
...
int simv2_RectilinearMesh_setCoordsXY(visit_handle h, visit_handle x, visit_handle y)
{
    visit_handle cHandles[2];
    cHandles[0] = x;
    cHandles[1] = y;
    return simv2_RectilinearMesh_setCoords_helper(h, cHandles, 2, "simv2_RectilinearMesh_setCoordsXY");
}

int simv2_RectilinearMesh_setCoordsXYZ(visit_handle h, visit_handle x, visit_handle y, visit_handle z)
{
    visit_handle cHandles[3];
    cHandles[0] = x;
    cHandles[1] = y;
    cHandles[2] = z;
    return simv2_RectilinearMesh_setCoords_helper(h, cHandles, 3, "simv2_RectilinearMesh_setCoordsXYZ");
}

int simv2_RectilinearMesh_setRealIndices(visit_handle h, int min[3], int max[3])
{
    int retval = VISIT_ERROR;
...
int simv2_RectilinearMesh_setBaseIndex(visit_handle h, int base_index[3])
{
    int retval = VISIT_ERROR;
...
int simv2_RectilinearMesh_setGhostCells(visit_handle h, visit_handle gz)
{
    int retval = VISIT_ERROR;
...
int simv2_RectilinearMesh_setGhostNodes(visit_handle h, visit_handle gn)
{
    int retval = VISIT_ERROR;
...
int simv2_RectilinearMesh_getCoords(visit_handle h, int *ndims, visit_handle *x, visit_handle *y, visit_handle *z)
{
    int retval = VISIT_ERROR;
...
int simv2_RectilinearMesh_getRealIndices(visit_handle h, int min[3], int max[3])
{
    int retval = VISIT_ERROR;
    ...
    if (obj != NULL) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            void *data = NULL;
            if (i < obj->ndims) {
                simv2_VariableData_getData(cHandles[i], owner, dataType, nComps, nTuples, data);
            }
...
int simv2_RectilinearMesh_getBaseIndex(visit_handle h, int base_index[3])
{
    int retval = VISIT_ERROR;
...
int simv2_RectilinearMesh_getGhostCells(visit_handle h, visit_handle *gz)
{
    int retval = VISIT_ERROR;
...
int simv2_RectilinearMesh_getGhostNodes(visit_handle h, visit_handle *gn)
{
    int retval = VISIT_ERROR;
...
int simv2_RectilinearMesh_check(visit_handle h)
{
    int retval = VISIT_ERROR;
    ...
    if (obj != NULL) {
        ...
        if (obj->ghostCells != VISIT_INVALID_HANDLE) {
            ...
            if (nCells != nTuples) {
                char tmp[1024] = {'\0'};
                snprintf(tmp, 1023,
                         "The number of elements in the ghost cell array %d does "
                         "not match the number of mesh cells %d.",
                         nTuples, nCells);
                VisItError(tmp);
                return VISIT_ERROR;
            }
