...
void VisIt_CurvilinearMesh::FreeCoordinates()
{
...
void VisIt_CurvilinearMesh::FreeGhostCells()
{
...
void VisIt_CurvilinearMesh::FreeGhostNodes()
{
...
static VisIt_CurvilinearMesh *GetObject(visit_handle h, const char *fname)
{
    char tmp[100];
...
static int simv2_CurvilinearMesh_setCoords_helper(visit_handle h, visit_handle *cHandles, int *dims, int ndims,
                                                  const char *fname)
{
    int retval = VISIT_ERROR;
    ...
    for (int i = 0; i < ndims; ++i) {
        if (nTuples[i] != nnodes) {
            snprintf(tmp, 80,
                     "The provided %s coordinates contain %d nodes "
...
int simv2_CurvilinearMesh_setCoordsXY(visit_handle h, int dims[2], visit_handle x, visit_handle y)
{
    visit_handle cHandles[2];
    cHandles[0] = x;
    cHandles[1] = y;
    return simv2_CurvilinearMesh_setCoords_helper(h, cHandles, dims, 2, "simv2_CurvilinearMesh_setCoordsXY");
}

int simv2_CurvilinearMesh_setCoordsXYZ(visit_handle h, int dims[3], visit_handle x, visit_handle y, visit_handle z)
{
    visit_handle cHandles[3];
    cHandles[0] = x;
    cHandles[1] = y;
    cHandles[2] = z;
    return simv2_CurvilinearMesh_setCoords_helper(h, cHandles, dims, 3, "simv2_CurvilinearMesh_setCoordsXYZ");
}

static int simv2_CurvilinearMesh_setAllCoords_helper(visit_handle h, int ndims, int dims[3], visit_handle coords,
                                                     const char *fname)
{
    // How many arrays make up the variable.
    int nArr = 1;
    ...
    if (nnodes != nTuples) {
        snprintf(tmp, 100,
                 "The product of the dimensions does not equal %d, "
                 "the number of tuples in the coordinate array",
                 nTuples);
        VisItError(tmp);
        return VISIT_ERROR;
    }
...
int simv2_CurvilinearMesh_setCoords2(visit_handle h, int dims[2], visit_handle coords)
{
    int tmp[3] = {0, 0, 1};
    tmp[0] = dims[0];
    tmp[1] = dims[1];
    return simv2_CurvilinearMesh_setAllCoords_helper(h, 2, tmp, coords, "simv2_CurvilinearMesh_setCoords2");
}

int simv2_CurvilinearMesh_setCoords3(visit_handle h, int dims[3], visit_handle coords)
{
    return simv2_CurvilinearMesh_setAllCoords_helper(h, 3, dims, coords, "simv2_CurvilinearMesh_setCoords3");
}

int simv2_CurvilinearMesh_setRealIndices(visit_handle h, int min[3], int max[3])
{
    int retval = VISIT_ERROR;
...
int simv2_CurvilinearMesh_setBaseIndex(visit_handle h, int base_index[3])
{
    int retval = VISIT_ERROR;
...
int simv2_CurvilinearMesh_setGhostCells(visit_handle h, visit_handle gz)
{
    int retval = VISIT_ERROR;
...
int simv2_CurvilinearMesh_setGhostNodes(visit_handle h, visit_handle gn)
{
    int retval = VISIT_ERROR;
...
int simv2_CurvilinearMesh_getCoords(visit_handle h, int *ndims, int dims[3], int *coordMode, visit_handle *x,
                                    visit_handle *y, visit_handle *z, visit_handle *c)
{
    int retval = VISIT_ERROR;
...
int simv2_CurvilinearMesh_getRealIndices(visit_handle h, int min[3], int max[3])
{
    int retval = VISIT_ERROR;
    ...
    if (obj != NULL) {
        for (int i = 0; i < 3; ++i) {
            ...
            max[i] = (obj->maxRealIndex[i] == -1) ? (obj->dims[i] - 1) : obj->maxRealIndex[i];
        }
        retval = VISIT_OKAY;
    }
    return retval;
...
int simv2_CurvilinearMesh_getBaseIndex(visit_handle h, int base_index[3])
{
    int retval = VISIT_ERROR;
...
int simv2_CurvilinearMesh_getGhostCells(visit_handle h, visit_handle *gz)
{
    int retval = VISIT_ERROR;
...
int simv2_CurvilinearMesh_getGhostNodes(visit_handle h, visit_handle *gn)
{
    int retval = VISIT_ERROR;
...
int simv2_CurvilinearMesh_check(visit_handle h)
{
    int retval = VISIT_ERROR;
