...
public:
    ...
    virtual avtPlot *AllocAvtPlot() { return eInfo->AllocAvtPlot(); }

    virtual bool ProvidesLegend() const { return eInfo->ProvidesLegend(); }

    virtual bool PermitsCurveViewScaling() const { return eInfo->PermitsCurveViewScaling(); }

    virtual bool Permits2DViewScaling() const { return eInfo->Permits2DViewScaling(); }

    virtual void InitializePlotAtts(AttributeSubject *atts, const avtPlotMetaData &plot)
    {
        eInfo->InitializePlotAtts(atts, plot);
    }

    virtual void ReInitializePlotAtts(AttributeSubject *atts, const avtPlotMetaData &plot)
    {
        eInfo->ReInitializePlotAtts(atts, plot);
    }

    virtual void ResetPlotAtts(AttributeSubject *atts, const avtPlotMetaData &plot)
    {
...
ViewerPlotPluginInfo *SimPlotPluginManager::GetViewerPluginInfo(const std::string &id)
{
    std::map<std::string, ViewerWrappedEnginePlotPluginInfo *>::iterator it;
    it = wrapped.find(id);
    if (it == wrapped.end()) {
