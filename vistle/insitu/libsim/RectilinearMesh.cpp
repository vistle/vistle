                                            ...
                                            bool vtkFormat)
{
    ...
    for (size_t iteration = 0; iteration < meshInfo.domains.size; iteration++) {
        ...
        v2check(simv2_invoke_GetMesh, meshInfo.domains.as<int>()[iteration], meshInfo.name);
        ...
        if (vtkFormat) {
            expandRectilinearToVTKStructured(data, dataTypeToVistle(meshArrays[0].type), dims,
                                             gridCoords);
            fillStructuredGridConnectivityList(dims,
                                               mesh->cl().begin() + totalNumElements * numCorners,
                                               totalNumVerts, VtkConListPattern{});

        } else {
            expandRectilinearToStructured(data, dataTypeToVistle(meshArrays[0].type), dims,
                                          gridCoords);
...
vistle::RectilinearGrid::ptr makeVistleMesh(const std::array<Array, 3> &meshData,
                                            message::SyncShmIDs &creator)
{
    vistle::RectilinearGrid::ptr mesh = creator.createVistleObject<vistle::RectilinearGrid>(
    meshData[0].size, meshData[1].size, meshData[2].size);
...
std::array<Array, 3> getMeshFromSim(const visit_handle &meshHandle)
{
    ...
    int ndims;
    v2check(simv2_RectilinearMesh_getCoords, meshHandle, &ndims, &coordHandles[0], &coordHandles[1],
            &coordHandles[2]);
