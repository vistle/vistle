...
bool vistle::insitu::libsim::readSim2File(const std::string& path, std::string& hostname, int& port, std::string& securityKey) {
...
bool vistle::insitu::libsim::sendInitToSim(const std::vector<std::string> launchArgs, const std::string& host, int port, const std::string& key) {
    ...
    if (ec) {
        std::cerr << "SendInitToSim failed to resolve query to host " << host << " on port " << port << ": " << ec.message() << std::endl;
        return false;
    }
    asio::connect(*s, endpoint_iterator, ec);
    if (ec) {
        std::cerr << "SendInitToSim failed to connect socket to host " << host << " on port " << port << ": " << ec.message() << std::endl;
        return false;
    }
