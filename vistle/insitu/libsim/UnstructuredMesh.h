...
void fillTypeConnAndElemLists(const visit_handle &meshHandle,
                              std::shared_ptr<vistle::UnstructuredGrid> grid);
struct connListData {
    Array data;
    int numElements = 0;
...
void allocateFields(std::shared_ptr<vistle::UnstructuredGrid> grid, size_t totalNumVerts,
                    size_t numVertices, size_t iteration, int numIterations,
                    std::array<vistle::Scalar *, 3> &gridCoords, size_t totalNumElements,
                    size_t numElements, int numCorners);
void setFinalFildSizes(std::shared_ptr<vistle::UnstructuredGrid> grid, size_t totalNumVerts,
                       size_t totalNumElements, int numCorners);
void fillRemainingFields(std::shared_ptr<vistle::UnstructuredGrid> grid, size_t totalNumElements,
                         int numCorners, int dim);

struct ConnectivityListPattern {
    ConnectivityListPattern();
    ConnectivityListPattern(vistle::Index (*getVertexIndex)(vistle::Index, vistle::Index,
                                                            vistle::Index, const vistle::Index[3]),
                            std::array<int, 3> loopOrder);
    vistle::Index (*vertexIndex)(vistle::Index, vistle::Index, vistle::Index,
                                 const vistle::Index[3]) = nullptr;
    std::array<int, 3> order = {0, 1, 2}; // Vistle's loop order is x, y, z
};
struct VtkConListPattern: ConnectivityListPattern {
    VtkConListPattern();
};
void fillStructuredGridConnectivityList(
const int *dims, vistle::Index *connectivityList, vistle::Index startOfGridIndex,
