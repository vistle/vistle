...
inline void v2check(std::function<int(Args1...)> fnct, Args2 &&... args) {
    ...
    auto numArgs1 = std::tuple_size<tuple1>(); // they must be > 0, because functions without arguments are handled by
                                               // the overload below
...
inline visit_handle v2check(visit_handle(fnct)()) { return v2check(std::function<visit_handle()>(fnct)); }
} // namespace insitu
} // namespace vistle
